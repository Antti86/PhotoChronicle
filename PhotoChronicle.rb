...
def main
  ...
  Dir.glob("**/*", File::FNM_DOTMATCH)
   .select do |f|
     File.file?(f) &&
     File.basename(f) != script_name &&                    # Don't move running script
     File.extname(f).downcase != ".exe" &&                 # Don't move exe files
     !excluded_paths.any? { |e| f.start_with?(e) } &&      # Exclude paths like .git
     f !~ %r{^\d{4}/}                                      # Already moved
  end
   .each do |file|
  ...
  puts "\nValmis. Paina Enter poistuaksesi..."
  gets
end
